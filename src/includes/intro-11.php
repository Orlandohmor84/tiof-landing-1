<!-- Intro Row -->
<div class="row intro-2-col" id="intro">
    <div class="col-xs-12 col-lg-4 order-2 order-lg-1" data-aos="fade-up">
    <img alt="man video recording with 6k camera" src="..\assets\img\intro\6kpro2-800x800.jpg">
    </div><!-- /.col -->
    <div class="col-xs-12 col-lg-8 order-1 order-lg-2" id="intro-triangle">
        <h2><strong>Fast and Affordable</strong><br>
        <span class="red">Faster than a 2-4-year degree, more powerful than others, able to be more affordable as a non-profit…</span><br>
        the super film & marketing degree including lifetime equipment,<br>
        <span class="red">work on 22 film productions and various marketing projects</span><br>
        Emmy-award-winning filmmakers/industry execs/mentors,<br>
        <span class="red">work with a caring interactive online studio team</span><br>
        help to start your film, marketing production co.<br>
        <span class="red">non-profit affordable tuition</span><br>
        graduation in 11 months</h2>
        <a role="button" class="btn" href="#next-steps">I WANT A DIGITAL FILM &AMP; MARKETING CAREER</a>
        <a role="button" class="btn btn-white" href="#next-steps">REQUEST FREE INFO</a>
    </div><!-- /.col -->
</div><!-- /.row -->